...
router.post("/webauthn-login-verification", async (req, res) => {
    ...
    let dbAuthenticator;
    const bodyCredIDBuffer = data.rawId;

    for (const dev of user.devices) {
        const currentCredential =  dev.credentialID;
        if (bodyCredIDBuffer === currentCredential) {
            dbAuthenticator = dev;
            break;
        }
    }
    ...
    try {
        const options = {
            ...
            credential: {
                ...
                publicKey: new Uint8Array(dbAuthenticator.credentialPublicKey.data),
...
router.post("/webauthn-registration-verification", async (req, res) => {
    ...
    if (verified && registrationInfo) {
        ...
        const existingDevice = user.devices ? user.devices.find(
            device =>  device.credentialID.equals(credentialID)
        ) : false;
